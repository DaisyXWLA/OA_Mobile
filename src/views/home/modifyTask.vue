<template>
  <div>
    <van-nav-bar fixed title="编辑任务" left-arrow @click-left="back"></van-nav-bar>
    <van-form @submit="onSubmit" style="margin-top:44px">
      ...
      <div class="copier">
        ...
        />
        <div class="member">
          <ul @click="showMemberOption = true">
...
<script>
import { update } from '@/api/task.js'
  ...
  methods: {
    getData(id) {
      ...
      findOne(params).then(resp => {
        if (resp.status === 200) {
          this.taskName = resp.body.taskName
          this.taskType = resp.body.taskTypeName
          this.assign = resp.body.executorName
          this.taskStartTime = resp.body.planStartTime
          this.taskEndTime = resp.body.planEndTime
          this.relatedProject = resp.body.projectInfoName
          this.priority = resp.body.priority
          this.description = resp.body.taskContent
          let memberArr = []
          if (resp.body.copyPersonName !== '') {
            memberArr = resp.body.copyPersonName.split(',')
          }
          ...
          this.member = resp.body.copyPersonName
          if (this.member !== '') {
            this.placeholder = ''
          } else {
    ...
    onSubmit() {
      const taskStartTimeFormat = new Date(this.taskStartTime.replace(/\./g, '/'))
      const taskEndTimeFormat = new Date(this.taskEndTime.replace(/\./g, '/'))
      if (taskStartTimeFormat.getTime() > taskEndTimeFormat.getTime()) {
        this.$toast.fail('任务结束时间必须大于任务开始时间')
      } else {
        const params = {
        ...
        update(params).then(resp => {
          if (resp.status === 200) {
            this.$toast.success('任务编辑成功')
          } else {
            this.$toast.fail('任务编辑失败')
          }
        })
      }
    },
